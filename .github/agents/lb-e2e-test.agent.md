...
1. Read the **test-conventions** skill — `read_file(".github/skills/test-conventions/SKILL.md")`
2. Read the **e2e-playwright-mcp** skill — `read_file(".github/skills/e2e-playwright-mcp/SKILL.md")`
3. Read at least one **existing sibling spec file** in the same `e2e/tests/desktop/` subdirectory to match the exact style
...
When a source component lacks a stable selector, add `data-testid` (and `aria-pressed` for toggle buttons) directly to the component. Keep changes minimal — only add attributes needed for testability.
